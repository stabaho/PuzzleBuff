--[[
    ...
    Behavior:
    - ONLY active out of combat (InCombatLockdown check everywhere).
    - When a unit has NO debuffs (NORMAL status) and we are NOT in combat,
      the MUF box is colored to indicate the highest-priority missing buff:
    ...
    - RIGHT-CLICK on a colored MUF casts the indicated buff on that unit.
      If in combat, right-click falls through to normal Decursive behavior.

    - LEFT-CLICK retains all normal Decursive debuff-cure behavior.

    - The periodic color refresh pauses automatically during combat.
    ...
    (no-debuff) units only when out of combat.
--]]
local addonName, T = ...;
...
function PuzzleBuff_Buffs_Init()
    ...
    -- (no debuffs) AND we are NOT in combat, override the color
    -- based on missing buffs. In combat, do nothing so Decursive
    -- keeps full control of its own UI.
    -- ----------------------------------------------------------
    local orig_SetColor = proto.SetColor;
    proto.SetColor = function(self)
        local result = orig_SetColor(self); -- run original first

        -- Out-of-combat only: recolor based on missing buffs
        if not InCombatLockdown() then
            ...
            -- In combat: always clear our tag so right-click is never
            -- intercepted and Decursive behaves exactly as normal
            self._puzzlebuff_buff = nil;
        end
    ...
    -- Intercept right-clicks ONLY when:
    --   1. Out of combat (InCombatLockdown() == false)
    --   2. The unit has a pending buff tag (_puzzlebuff_buff set)
    -- In all other cases fall through to Decursive's original
    ...
    MUF.OnPreClick = function(frame, Button)
        ...
        if Button == "RightButton"
            ...
            C_Timer.After(0.5, function()
                if muf and muf.CurrUnit and not InCombatLockdown() then
        ...
        -- (covers: in combat, left-click, debuffed units, etc.)
        return orig_OnPreClick(frame, Button);
    end;

    -- ----------------------------------------------------------
    -- Periodic refresh: re-evaluate buff colors every 3s
    -- Skips entirely when in combat so we never interfere with
    -- Decursive's combat UI or cause any performance overhead.
    -- ----------------------------------------------------------
    local ticker     = CreateFrame("Frame");
    local elapsed_acc = 0;
    ticker:SetScript("OnUpdate", function(self, elapsed)
        -- Hard stop: do nothing during combat
        if InCombatLockdown() then
            elapsed_acc = 0; -- reset timer so we don't fire immediately on exit
            return;
        end
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cff00ccff[PuzzleBuff]|r Buff system active (out-of-combat only). Right-click MUFs to cast missing Priest buffs.");
