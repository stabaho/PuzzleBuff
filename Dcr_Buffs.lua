...
waitFrame:SetScript("OnEvent", function(self, event)
    ...
    if not (T and T.Dcr and T.Dcr.MicroUnitF and T.Dcr.MicroUnitF.prototype) then
        DEFAULT_CHAT_FRAME:AddMessage("|cffff4444[DecursivePlus]|r ERROR: Decursive core not found. Buff system disabled.");
        return;
    end
...
-- SPELL DEFINITIONS
-- ============================================================
-- Spell IDs for TBC Classic (Classic Era equivalents also listed)
...
function DcrPlus_Buffs_Init()
    ...
    proto.SetColor = function(self)
        ...
        -- (UnitStatus == NORMAL = 1 in Decursive's constants)
        local DC = T._C;
        if self.UnitStatus == DC.NORMAL and UnitExists(self.CurrUnit) then
            ...
            if missing then
                ...
                self._dcrplus_buff = missing;
            else
                -- All buffs present: restore normal gray color and clear tag
                self._dcrplus_buff = nil;
            end
        else
            -- Not NORMAL status: clear our buff tag so right-click
            -- falls through to normal Decursive behavior
            self._dcrplus_buff = nil;
        end
    ...
    -- ----------------------------------------------------------
    -- HOOK  MicroUnitF:OnPreClick  (the STATIC method, not proto)
    -- We intercept right-clicks when the unit has a pending buff.
    -- ----------------------------------------------------------
    local orig_OnPreClick = MUF.OnPreClick;
    MUF.OnPreClick = function(frame, Button)
        local muf = frame.Object;

        -- Only intercept right-click on a NORMAL unit that has a
        -- pending buff.  Everything else falls through unchanged.
        if Button == "RightButton"
           and muf
           and muf._dcrplus_buff
           and not InCombatLockdown() then

            local buff   = muf._dcrplus_buff;
            local unit   = muf.CurrUnit;

            -- Cast the spell by name (works in Classic/TBC) or by ID
            CastSpellByName(buff.name, unit);

            -- Force an immediate re-check of this MUF's color
            -- (the buff will appear in ~1 server tick)
    ...
    -- ----------------------------------------------------------
    -- Periodic refresh: re-evaluate buff colors every 3s so the
    -- boxes update even without clicking (e.g. buff expires).
    -- ----------------------------------------------------------
    ...
    ticker:SetScript("OnUpdate", function(self, elapsed)
        ...
        -- Trigger a lightweight SetColor-only update on all shown MUFs
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cff00ccff[DecursivePlus]|r Buff system active. Right-click MUFs to cast missing Priest buffs.");
